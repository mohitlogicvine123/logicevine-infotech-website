    ...
    <table class="table-auto border-collapse w-full whitespace-nowrap text-left text-gray-500 dark:text-dark-text font-medium">
        <thead>
            <tr>
                ...
                <th class="p-6 py-4 bg-[#F2F4F9] dark:bg-dark-card-two dk-border-one">Experience</th>
            ...
            <tr>
                ...
                <td class="p-6 py-4 dk-border-one">
                    <div class="flex items-center gap-2">
                     <a href="{{ route('carreer.edit', $exp->id) }}" class="btn-icon btn-primary-icon-light size-7 btn b-outline btn-success-outline">
                    <i class="ri-edit-line text-inherit text-[13px]"></i>
                     </a>
                        <form action="{{ route('carreer.destroy', $exp->id) }}" method="POST" onsubmit="return confirm('Are you sure?')">
                            @csrf
                            @method('DELETE')
