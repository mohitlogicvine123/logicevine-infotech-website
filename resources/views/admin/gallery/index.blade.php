    ...
    <table class="table-auto border-collapse w-full whitespace-nowrap text-left text-gray-500 dark:text-dark-text font-medium">
        <thead>
            <tr>
                ...
                <th class="p-6 py-4 bg-[#F2F4F9] dark:bg-dark-card-two dk-border-one">Img-Alt</th>
        ...
        <tbody class="divide-y divide-gray-200 dark:divide-dark-border-three">
            ...
            <tr>
                <td class="p-6 py-4 dk-border-one">{{ $img->id }}</td>
                <td class="p-6 py-4 dk-border-one">{{ $img->imagealt }}</td>
                 <td>
                 <img src="{{asset('storage/'.$img->image)}}" alt="ind Image" style="width:100px;">
                 </td>

                <td class="p-6 py-4 dk-border-one">
                    <div class="flex items-center gap-2">
                     <a href="{{ route('gallery.edit', $img->id) }}" class="btn-icon btn-primary-icon-light size-7 btn b-outline btn-success-outline">
                    <i class="ri-edit-line text-inherit text-[13px]"></i>
                     </a>
                        <form action="{{ route('gallery.destroy', $img->id) }}" method="POST" onsubmit="return confirm('Are you sure?')">
                            @csrf
                            @method('DELETE')
