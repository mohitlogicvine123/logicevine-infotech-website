            ...
            <tr>
                ...
                <td class="p-6 py-4 dk-border-one">
                    <div class="flex items-center gap-2">
                     <a href="{{ route('industries.edit', $ind->id) }}" class="btn-icon btn-primary-icon-light size-7 btn b-outline btn-success-outline">
                    <i class="ri-edit-line text-inherit text-[13px]"></i>
                     </a>
                        <form action="{{ route('industries.destroy', $ind->id) }}" method="POST" onsubmit="return confirm('Are you sure?')">
                            @csrf
                            @method('DELETE')
